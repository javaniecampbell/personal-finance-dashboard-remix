...
import { getUpcomingBills, getBillHistory, scheduleBillPayment } from '~/utils/bills.server';
...
export const action = async ({ request }) => {
  ...
  await scheduleBillPayment(userId, billId, amount, paymentDate);

  return redirect('/bills');
};

const BillPaymentForm = ({ bill, onSubmit }) => {
...
export default function BillPaymentPage() {
  const { upcomingBills, billHistory } = useLoaderData();
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {upcomingBills.map((bill) => (
            <div key={bill.id} className="bg-white shadow rounded-lg p-6">
              ...
              <p className="text-gray-600 mb-1">Due: {formatDate(bill.dueDate)}</p>
              <p className="text-2xl font-bold mb-4">{formatCurrency(bill.amount)}</p>
              <button
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" id="my-modal">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3 text-center">
              <h3 className="text-lg leading-6 font-medium text-gray-900">Pay {selectedBill.name}</h3>
              <div className="mt-2 px-7 py-3">
                <BillPaymentForm bill={selectedBill} onSubmit={handlePaymentSubmit} />
              </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {billHistory.map((payment) => (
                <tr key={payment.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{payment.billName}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatCurrency(payment.amount)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(payment.datePaid)}</td>
