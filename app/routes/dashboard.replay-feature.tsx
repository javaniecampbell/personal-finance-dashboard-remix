...
const financialEventTypes = ['DEPOSIT', 'WITHDRAWAL', 'BUDGET_ADJUSTMENT', 'BILL_PAYMENT'];

const generateMockEvents = (count) => {
  return Array.from({ length: count }, (_, index) => ({
    id: index + 1,
    type: financialEventTypes[Math.floor(Math.random() * financialEventTypes.length)],
...
const Dashboard = () => {
  ...
  const { recordEvent, startReplay, stopReplay } = useReplay();
      ...
      <div className="mb-6">
        ...
        <ul className="space-y-2">
          {recentEvents.map((event) => (
            <li key={event.id} className="flex justify-between items-center bg-white p-3 rounded shadow">
      ...
      <div className="mb-6">
        <button
          ...
          className={`flex items-center px-4 py-2 rounded ${
            isReplaying ? 'bg-red-500 hover:bg-red-600' : 'bg-blue-500 hover:bg-blue-600'
          } text-white`}
        >
          {isReplaying ? (
            <>
      ...
      {currentReplayEvent && (
        <div className="bg-yellow-100 border-l-4 border-yellow-500 p-4 mb-6">
          <div className="flex items-center">
            ...
            <p>
              Replaying: <strong>{currentReplayEvent.type}</strong> - Amount: ${currentReplayEvent.amount}
            </p>
          </div>
        </div>
      )}
